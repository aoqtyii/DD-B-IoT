...
class BlockChainEnv(gym.Env, utils.EzPickle):
    ...
    def __init__(self, n_of_nodes=N_OF_NODES, area_of_nodes=AREA_OF_NODES,
        ...
        self.observation_space = spaces.Dict(
            ...
            'location_of_producers': spaces.Box(low=-AREA_OF_NODES[0]/2,high=AREA_OF_NODES[1]/2,shape=(self.n_of_block_producer,2)),
        })
        # -----------------------------------------------------------------------------------------------------
    ...
    def step(self, action):
        ...
        self.concate = np.asarray([[self.producer_X[i][0], self.producer_Y[i][0]] for i in range(self.n_of_block_producer)], dtype=np.float32)
        ...
        # 单位B/s，计算使用最终达到一致性时间
        throughout = np.floor(action['block_size'] / self.average_transaction_size) * 1000 / (action['block_interval'] + T_final)
    ...
    def get_computing_capacity(self, action, transmission_rate):
        ...
        if action['no_consensus_algorithm'] != -1:
            ...
            Td2 = min(max(M*action['block_size']*8*0.001/[transmission_rate[arg_primary][i] for i in iop if i not in[arg_primary, arg_client]]), Tau)
            Td3 = min(max(M*action['block_size']*8*0.001/[transmission_rate[i][j] for i in iop for j in iop if i != arg_client and j != arg_client and i != j]), Tau)
            Td4 = min(max(M*action['block_size']*8*0.001/[transmission_rate[i][j] for i in iop for j in iop if i != j]), Tau)
            Td5 = min(max(M*action['block_size']*8*0.001/[transmission_rate[i][arg_client] for i in iop if i != arg_client]), Tau)
            ...
            Td3 = min(max((M * action['block_size']*8*0.001 / transmission_rate[i][arg_client] for i in iop if i not in [arg_client])), Tau)

            Td4 = min(max((M * action['block_size']*8*0.001 / transmission_rate[arg_client][i] for i in iop if i not in [arg_client])), Tau)

            Td5 = min(max((M * action['block_size']*8*0.001 / transmission_rate[i][arg_client] for i in iop if i not in [arg_client])), Tau)
            ...
            Td1 = min(max(action['block_size']*8*0.001 / transmission_rate[arg_client][i] for i in iop if i != arg_client), Tau)
